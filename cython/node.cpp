...
UCTNode::UCTNode(Game* game, int move, UCTNode* parent, bool selfplay)
{
	this->game = game;
	this->move = move;
	this->parent = std::make_shared<UCTNode>(parent);
	this->self_play = selfplay;
	is_expanded = 0;
	action_size = game->action_size;
	children = std::unordered_map<int, std::shared_ptr<UCTNode>>();
...
std::shared_ptr<UCTNode> UCTNode::select_leaf()
{
	std::shared_ptr<UCTNode> current = shared_from_this();
	int bestmove = 0;
...
std::shared_ptr<UCTNode> UCTNode::try_add_child(int move)
{
	if (contains(children, move))
	{
		Game* copy_game = game->copyGame();
		copy_game->makeMove(move);
		children[move] = std::make_shared<UCTNode>(copy_game, move, shared_from_this(), self_play);
	}
